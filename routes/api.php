...
Route::prefix('v1')->group(function () use ($ctrl) {
    ...
    Route::group(['middleware' => ['auth:sanctum']], function () use ($ctrl) {
        ...
        Route::get('cases/{id}',                        [$ctrl['Case'], 'show']);   //< show a case; accessible by: teachers + student (reporter)
        Route::patch('cases/{id}',                      [$ctrl['Case'], 'update']);     //< update a case; accessible by: teachers + student (reporter)
        Route::delete('cases/{id}',                     [$ctrl['Case'], 'destroy']);    //< delete a case; accessible by: teachers + student (reporter)
        Route::get('cases/{id}/reports',                [$ctrl['Case'], 'reports']);    //< list all reports in the case; accessible by: teachers + student (reporter)
